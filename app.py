...
x_min = st.sidebar.slider('Límite inferior de x', -10.0, 0.0, -6.5)
x_max = st.sidebar.slider('Límite superior de x', 0.0, 10.0, 6.5)
y_min = st.sidebar.slider('Límite inferior de y', -10.0, 0.0, -6.5)
y_max = st.sidebar.slider('Límite superior de y', 0.0, 10.0, 6.5)

# Parámetros para optimización
st.sidebar.header('Métodos de optimización')
method = st.sidebar.selectbox('Selecciona un método', ['GD', 'SGD', 'RMSprop', 'Adam'])

# Configuración del método
eta = st.sidebar.slider('Tasa de aprendizaje (eta)', 0.001, 0.1, 0.01)
epochs = st.sidebar.slider('Número de épocas', 1000, 20000, 10000)

# Parámetros adicionales para RMSprop y Adam
if method == 'RMSprop':
    decay = st.sidebar.slider('Factor de decaimiento (decay)', 0.1, 1.0, 0.9)
elif method == 'Adam':
    beta1 = st.sidebar.slider('Beta1 (Adam)', 0.1, 1.0, 0.9)
    beta2 = st.sidebar.slider('Beta2 (Adam)', 0.1, 1.0, 0.999)
    alpha = st.sidebar.slider('Alpha (Adam)', 0.001, 0.1, 0.001)
